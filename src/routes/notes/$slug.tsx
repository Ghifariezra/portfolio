...
export const Route = createFileRoute("/notes/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!note) {
      return {
        meta: [
          ...
          { name: "description", content: "The requested technical note could not be found." }
        ]
      };
    }
    ...
    const description = note.description || "Technical note and exploration by Ghifari Ezra Ramadhan.";
    const url = `https://ezdev.xyz/notes/${note.slug}`;
    const keywords = note.tags?.map(t => t.name).join(", ") || "software development, tech notes, data engineering";
    ...
    return {
      meta: [
        ...
        // Twitter Card
        { name: "twitter:card", content: note.image ? "summary_large_image" : "summary" },

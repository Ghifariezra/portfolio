...
export const Route = createFileRoute("/projects/$slug")({
	...
	head: ({ loaderData }) => {
		...
		const description =
			project.description ||
			"Portfolio project built by Ghifari Ezra Ramadhan.";
		const url = `https://ezdev.xyz/projects/${project.slug}`;
		const keywords =
			project.tags?.map((t) => t.name).join(", ") ||
			"portfolio, software development, web project, ghifari ezra";
		...
		return {
			meta: [
				...
				{
					name: "twitter:card",
					content: project.image ? "summary_large_image" : "summary",
				},

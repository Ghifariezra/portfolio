...
const CUSTOM_LANGS = async (
	h: Awaited<ReturnType<typeof createHighlighter>>
) => {
	const raw = Array.isArray(jsonGrammar) ? jsonGrammar[0] : jsonGrammar;
	await h.loadLanguage({
		...raw,
		name: "file-tree",
		aliases: ["filetree", "folder-structure"],
	} as any);
	await h.loadLanguage({
		...raw,
		name: "react-flow",
		aliases: ["react-flow", "erd", "reactflow"],
	} as any);
	return h;
};

const lightHighlighterPromise = createHighlighter({
	themes: ["github-light"],
	langs: [...LANGS],
}).then(CUSTOM_LANGS);
const darkHighlighterPromise = createHighlighter({
	themes: ["github-dark"],
	langs: [...LANGS],
}).then(CUSTOM_LANGS);
	...
	return BlockNoteSchema.create().extend({
		blockSpecs: {
			codeBlock: createCodeBlockSpec({
				...
				createHighlighter: () =>
					isDark ? darkHighlighterPromise : lightHighlighterPromise,
			}),
		},
	});
}

...
export const RouteComponent = memo(function RouteComponent() {
    ...
    const { data: response, isLoading, isError } = publicActions.useGetProjectBySlug(slug);
        ...
        return (
            <div className="flex min-h-[70vh] w-full items-center justify-center flex-col gap-6">
                <div className="border-4 border-foreground p-8 rounded-xl bg-card shadow-brutal text-center max-w-md">
                    <p className="font-heading text-3xl font-bold text-foreground mb-2">Project Not Found</p>
                    <p className="font-sans text-muted-foreground mb-6">The project you are looking for does not exist or is currently private.</p>
                    <Link
                        to="/projects"
            ...
            <header className="mb-12">
                <div className="flex flex-wrap items-center justify-between gap-6 mb-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded font-mono text-[10px] font-bold tracking-wider uppercase border-2 shadow-sm ${isCompleted
                    ...
                    {project.embed_url && (
                        ...
                        >
                            {project.embed_type === 'github' ? <GithubLogo size={16} weight="fill" /> : <Globe size={16} weight="bold" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 p-8 border-2 border-border rounded-xl bg-card shadow-brutal dark:shadow-none">
                ...
                <div className="col-span-1 md:col-span-2">
                    <div className="flex justify-between items-center mb-3">
                        <span className="font-mono text-xs font-bold text-muted-foreground tracking-wider uppercase">Completion Progress</span>
                        <span className="font-mono text-sm font-bold text-foreground">{project.progress}%</span>
                    </div>
                    ...
                    <div className="col-span-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {project.collaborators.map(collab => (
                                <span key={collab.id} className="font-sans text-sm font-semibold text-foreground bg-background px-3 py-1.5 border-2 border-border rounded-md shadow-sm">
                                    {collab.name}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {project.tags && project.tags.length > 0 ? (
                            project.tags.map(tag => (
                                <span key={tag.id} className="font-mono text-[10px] font-bold uppercase tracking-wider text-foreground bg-muted px-3 py-1.5 rounded border-2 border-border shadow-sm hover:-translate-y-0.5 transition-transform cursor-default">
                                    {tag.name}
                                </span>
                            ))
                        ) : (
                            <span className="font-mono text-xs text-muted-foreground">No tags recorded</span>
                        )}
                    </div>
                </div>
            </div>

...
import { ArrowUUpLeft, CalendarBlank, Clock, Spinner } from "@phosphor-icons/react";
...
export const ProjectForm = memo(function ProjectForm({
    ...
    const defaultValues = isEditMode ? initialData : { user_id: profile?.id, ...initialData };
    const { form, loading } = useProjectForm(defaultValues, initialSchedule);

    const { data: catRes, isLoading: catLoading } = taxonomyActions.useGetCategories();
    ...
    const { data: colRes, isLoading: colLoading } = collaboratorActions.useGetCollaborators();
                        ...
                        <form.Field name="tag_ids">
                            {(field) => (
                                <div className="mt-8 p-6 bg-card border-2 border-border rounded-md shadow-brutal-sm dark:shadow-none">
                                    <div className="font-mono text-[11px] font-bold uppercase tracking-widest text-foreground mb-4">Project Tags</div>
                                    {tagLoading ? (
                            ...
                            {(field) => (
                                <div>
                                    ...
                                    >
                                        ...
                                        {categories.map((cat) => (
                                            <option key={cat.id as string} value={cat.id as string}>{cat.name}</option>
                                        ))}
                                    </select>
                                </div>
                            )}
                                                ...
                                                {colLoading ? (
                                                    ...
                                                    <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-2">
                                                        {collaborators.map((col) => (
                                                            <label key={col.id as string} className="flex items-center gap-2 cursor-pointer group">
                                                                <input
                                                                    type="checkbox"
                                                                    checked={field.state.value.includes(col.id as string)}
                                                                    onChange={(e) => {
                                                                        if (e.target.checked) {
                                                                            field.handleChange([...field.state.value, col.id as string]);
                                                                        } else {
                                                                            field.handleChange(field.state.value.filter((id) => id !== (col.id as string)));
                                                                        }
                                                                    }}
                                                                ...
                                                                <span className="font-sans text-sm text-foreground group-hover:text-primary transition-colors">
                                                                    {col.name}{" "}
                                                                    <span className="text-muted-foreground text-xs">({col.role || "Member"})</span>
                                                                </span>
                                                            </label>
                                                        ))}
                                                    </div>
                                    ...
                                    publishStatus === "scheduled" ? (
                                        <div className="space-y-4 p-4 border-2 border-dashed border-border rounded-md bg-accent/5">
                                            ...
                                            <form.Field name="_publish_date">
                                                {(field) => (
                                                    <div>
                                                        <label htmlFor={field.name} className="font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">

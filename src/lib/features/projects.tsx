...
function ProgressBar({
    ...
    progress,
    label,
}: { status: ProjectStatus; progress: number; label: string }) {
    return (
        <div className="space-y-2">
            <div className="flex justify-between items-center">
                ...
                <span
                    className={`font-mono text-[11px] font-semibold ${status === "active"
                            ? "text-primary"
                            : status === "stable"
                                ? "text-green-400"
                                : status === "beta"
...
function FilterButton({
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className={`px-5 py-2 rounded font-mono text-[11px] font-semibold tracking-wider uppercase transition-all duration-200 cursor-pointer ${active

...
export const RouteComponent = memo(function RouteComponent() {
	...
	const {
		...
		isLoading,
		isError,
	} = publicActions.useGetNoteBySlug(slug);
	...
	const primaryCategory =
		note.type_language ||
		(note.tags && note.tags.length > 0 ? note.tags[0].name : "General");
			...
			<footer className="pt-8 border-t-2 border-border flex flex-wrap gap-2 items-center">
				...
				{note.tags && note.tags.length > 0 ? (
					note.tags.map((tag) => (
						<span
							key={tag.id}
							className="font-mono text-[10px] font-bold uppercase tracking-wider text-foreground bg-muted px-3 py-1.5 rounded border-2 border-border shadow-sm hover:-translate-y-0.5 transition-transform"
						>
							{tag.name}
						</span>
					))
